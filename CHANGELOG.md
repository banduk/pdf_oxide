...
- **`subset_pages` exposed in the Python and C bindings** — `PdfDocument.subset_pages(pages)` (PyO3) and `document_editor_subset_pages_to_bytes` (C ABI, the base for the other language bindings) return the subset as PDF bytes.
- **Multi-source rebuild (`PdfRebuilder`) exposed in Python** — `PdfRebuilder` (PyO3) merges pages from several source PDFs into one, copying only what's needed and deduplicating shared objects: `add_source(bytes) -> index`, `add_page(src, page)`, `add_pages(src, pages)`, `build() -> bytes`. Its constructor takes the same fidelity knobs as `subset_pages` (`resources`, `on_signature`, `keep_*`, `dedup`).
- **Subset fidelity is configurable by level, and fully exposed in Python** — `SubsetOptions.resources` is a `ResourceTrim` level (`Wholesale` = copy resources as-is/fastest, `Page` = trim page-level only, `Forms` = also trim Form XObjects/default; replaces the old `trim_forms` bool), and `SignaturePolicy` gains `Drop` (remove the seal too) alongside `PreserveVisual`/`Refuse`. Every knob is reachable from Python via `PdfDocument.subset_pages(pages, dedup=…, resources="…", on_signature="…", keep_links=…, keep_outlines=…, keep_struct_tree=…, keep_acroform=…, keep_optional_content=…, keep_catalog_metadata=…)`, so callers can trade fidelity for speed per layer.
- **Subset fidelity for interactive forms, layers, and metadata** — subsetting now preserves the `/AcroForm` interactive form (the `/Fields` tree is pruned to fields whose widgets land on kept pages and relinked, keeping field types/values/`/DR`/`/DA`, so the form stays editable rather than silently flattening), optional-content layers (`/OCProperties`), and position-independent catalog metadata (`/Lang`, XMP `/Metadata`, `/ViewerPreferences`, `/PageMode`, `/PageLayout`). Each is individually toggleable via `SubsetOptions` (`keep_acroform`, `keep_optional_content`, `keep_catalog_metadata`, all default on); `SubsetReport.form_fields` reports how many fields survived.
